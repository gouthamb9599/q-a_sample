...
DetailsController.login = (params, res) => {
    client.query(`select * from details where email=$1 and password=$2`, [params.email, params.password],
        (err, results) => {
...
DetailsController.signup = (params, res) => {
...
DetailsController.googlelogin = (params, res) => {
    const password = "";
    client.query('select * from details where email=$1 and token=$2', [params.email, params.token],
        (err, results) => {
            if (err) console.log(err);
            else {
                // console.log(results);
                if (results.rowCount === 1) {
                    ...
                    // console.log(token)
                    res.send({ success: true, token, Name: params.Name, id: results.rows[0].id });
                }
                else {
